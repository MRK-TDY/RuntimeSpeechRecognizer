...
static const ggml_type_traits_t type_traits[GGML_TYPE_COUNT] = {
    /*[GGML_TYPE_F32] = */{
        ...
        /*.is_quantized             =*/ false, ggml_to_float_t(), ggml_from_float_t(), ggml_from_float_t(),
    ...
    /*[GGML_TYPE_Q8_1] = */{
        ...
        /*.is_quantized             =*/ true, ggml_to_float_t(),
        /*.from_float               =*/ quantize_row_q8_1,
        /*.from_float_reference     =*/ (ggml_from_float_t) quantize_row_q8_1_reference, ggml_vec_dot_t(),
        /*.vec_dot_type             =*/ GGML_TYPE_Q8_1,
        /*.nrows                    =*/ 1,
    },
    /*[GGML_TYPE_Q2_K] = */{
    ...
    /*[GGML_TYPE_F64] = */{
        ...
        /*.is_quantized             =*/ false, ggml_to_float_t(), ggml_from_float_t(), ggml_from_float_t(), ggml_vec_dot_t(), ggml_type(),
        /*.nrows                    =*/ 1,
    },
    /*[GGML_TYPE_IQ1_M] = */{
        /*.type_name                =*/ "iq1_m",
...
void ggml_numa_init(enum ggml_numa_strategy numa_flag) {
    ...
    // set numa scheme
    g_state_ggml.numa.numa_strategy = numa_flag;

    GGML_PRINT_DEBUG("numa strategy %u\n",g_state_ggml.numa.numa_strategy);

    g_state_ggml.numa.cpuset = ggml_get_numa_affinity();

    // enumerate nodes
    while (g_state_ggml.numa.n_nodes < GGML_NUMA_MAX_NODES) {
        rv = snprintf(path, sizeof(path), "/sys/devices/system/node/node%u", g_state_ggml.numa.n_nodes);
        GGML_ASSERT(rv > 0 && (unsigned)rv < sizeof(path));
        if (stat(path, &st) != 0) { break; }
        ++g_state_ggml.numa.n_nodes;
    }

    // enumerate CPUs
    while (g_state_ggml.numa.total_cpus < GGML_NUMA_MAX_CPUS) {
        rv = snprintf(path, sizeof(path), "/sys/devices/system/cpu/cpu%u", g_state_ggml.numa.total_cpus);
        GGML_ASSERT(rv > 0 && (unsigned)rv < sizeof(path));
        if (stat(path, &st) != 0) { break; }
        ++g_state_ggml.numa.total_cpus;
    }

    GGML_PRINT_DEBUG("found %u numa nodes, %u CPUs\n", g_state_ggml.numa.n_nodes, g_state_ggml.numa.total_cpus);

    // figure out which node we're on
    uint current_cpu;
    int getcpu_ret = 0;
#if __GLIBC__ > 2 || (__GLIBC__ == 2 && __GLIBC_MINOR__ > 28) || defined(__COSMOPOLITAN__)
    getcpu_ret = getcpu(&current_cpu, &g_state_ggml.numa.current_node);
#else
    // old glibc doesn't have a wrapper for this call. Fall back on direct syscall
#   if !defined(SYS_getcpu) && defined(SYS_get_cpu)
#       define SYS_getcpu SYS_get_cpu // some older glibc versions use this name
#   endif
    getcpu_ret = syscall(SYS_getcpu, &current_cpu, &g_state_ggml.numa.current_node);
#endif

    if (g_state_ggml.numa.n_nodes < 1 || g_state_ggml.numa.total_cpus < 1 || getcpu_ret != 0) {
        g_state_ggml.numa.n_nodes = 0;
        return;
    }

    GGML_PRINT_DEBUG("found our process on numa node %u, CPU %u\n", g_state_ggml.numa.current_node, current_cpu);

    for (uint32_t n = 0; n < g_state_ggml.numa.n_nodes; ++n) {
        struct ggml_numa_node * node = &g_state_ggml.numa.nodes[n];
        GGML_PRINT_DEBUG("CPUs on node %u:", n);
        node->n_cpus = 0;
        for (uint32_t c = 0; c < g_state_ggml.numa.total_cpus; ++c) {
...
bool ggml_is_numa(void) {
    return g_state_ggml.numa.n_nodes > 1;
}
    ...
    if (is_first_call) {
        ...
        // initialize g_state_ggml
        {
            ...
            for (int i = 0; i < GGML_MAX_CONTEXTS; ++i) {
                g_state_ggml.contexts[i].used = false;
            }

            const uint64_t t_end = ggml_time_us(); UNUSED(t_end);

            GGML_PRINT_DEBUG("%s: g_state_ggml initialized in %f ms\n", __func__, (t_end - t_start)/1000.0f);
        }

        is_first_call = false;
    }

    // find non-used context in g_state_ggml
    struct ggml_context * ctx = NULL;

    for (int i = 0; i < GGML_MAX_CONTEXTS; i++) {
        if (!g_state_ggml.contexts[i].used) {
            g_state_ggml.contexts[i].used = true;
            ctx = &g_state_ggml.contexts[i].context;
...
void ggml_free(struct ggml_context * ctx) {
    ...
    for (int i = 0; i < GGML_MAX_CONTEXTS; i++) {
        if (&g_state_ggml.contexts[i].context == ctx) {
            g_state_ggml.contexts[i].used = false;
    ...
    if (ith == 0) {
        ...
        for (int64_t iid1 = 0; iid1 < ids->ne[1]; ++iid1) {
            for (int id = 0; id < n_ids; ++id) {
                ...
                MMID_MATRIX_ROW(i02, matrix_row_counts[i02]) = mmid_row_mapping {(int32_t)id, (int32_t)iid1};
...
static void ggml_compute_forward_get_rows_f16(
    ...
    for (int64_t i = ir0; i < ir1; ++i) {
        ...
        ggml_fp16_to_fp32_row(
                (const ggml_fp16_t *)(const void *) ((char *) src0->data + i01*nb01 + i11*nb02 + i12*nb03),
                     (float *) ((char *)  dst->data + i10*nb1  + i11*nb2  + i12*nb3), nc);
    }
}
...
static void ggml_compute_forward_get_rows_bf16(
    ...
    for (int64_t i = ir0; i < ir1; ++i) {
        ...
        ggml_bf16_to_fp32_row(
                (const ggml_bf16_t *)(const void *) ((char *) src0->data + i01*nb01 + i11*nb02 + i12*nb03),
                     (float *) ((char *)  dst->data + i10*nb1  + i11*nb2  + i12*nb3), nc);
    }
}
...
static struct hash_map * ggml_new_hash_map(size_t size) {
    struct hash_map * result = (hash_map *)GGML_MALLOC(sizeof(struct hash_map));
    result->set = ggml_hash_set_new(size);
    result->vals = (ggml_tensor **)GGML_MALLOC(sizeof(struct ggml_tensor *) * result->set.size);
...
static void set_numa_thread_affinity(int thread_n) {
    ...
    int node_num;
    int rv;
    size_t setsize = CPU_ALLOC_SIZE(g_state_ggml.numa.total_cpus);

    switch(g_state_ggml.numa.numa_strategy) {
        case GGML_NUMA_STRATEGY_DISTRIBUTE:
            // run thread on node_num thread_n / (threads per node)
            node_num = thread_n % g_state_ggml.numa.n_nodes;
            break;
        case GGML_NUMA_STRATEGY_ISOLATE:
            // run thread on current_node
            node_num = g_state_ggml.numa.current_node;
            break;
        case GGML_NUMA_STRATEGY_NUMACTL:
            // use the cpuset that numactl gave us
            rv = pthread_setaffinity_np(pthread_self(), setsize, &g_state_ggml.numa.cpuset);
            if (rv) {
    ...
    struct ggml_numa_node * node = &g_state_ggml.numa.nodes[node_num];

    cpu_set_t * cpus = CPU_ALLOC(g_state_ggml.numa.total_cpus);
...
static void clear_numa_thread_affinity(void) {
    ...
    cpu_set_t * cpus = CPU_ALLOC(g_state_ggml.numa.total_cpus);
    CPU_ZERO_S(setsize, cpus);
    for (unsigned i = 0; i < g_state_ggml.numa.total_cpus; ++i) {
...
static enum ggml_opt_result ggml_opt_adam(
    ...
    float * pf = (float *)(params.past > 0 ? opt->adam.pf->data : NULL); // past function values
...
static enum ggml_opt_result ggml_opt_lbfgs(
    ...
    float * pf = (float *)(params.past > 0 ? opt->lbfgs.pf->data : NULL); // past function values
...
static const size_t GGUF_TYPE_SIZE[GGUF_TYPE_COUNT] = {
    ...
    /*[GGUF_TYPE_STRING]  =*/ sizeof(struct gguf_str), 0,
...
struct gguf_context * gguf_init_from_file(const char * fname, struct gguf_init_params params) {
    ...
    if (ctx->header.n_tensors > 0) {
        ctx->infos = (struct gguf_tensor_info *)GGML_CALLOC(ctx->header.n_tensors, sizeof(struct gguf_tensor_info));
            ...
            case GGUF_TYPE_ARRAY:
                {
                    if (src->kv[i].value.arr.type == GGUF_TYPE_STRING) {
                        const char ** data = (const char **)GGML_CALLOC(src->kv[i].value.arr.n, sizeof(char *));
...
void gguf_add_tensor(
    ...
    ctx->infos = (struct gguf_tensor_info *)realloc(ctx->infos, (idx + 1)*sizeof(struct gguf_tensor_info));
