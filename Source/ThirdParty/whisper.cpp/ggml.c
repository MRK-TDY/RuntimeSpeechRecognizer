...
static struct ggml_tensor * ggml_acc_impl(
    ...
    int32_t params[] = { (int32_t)nb1, (int32_t)nb2, (int32_t)nb3, (int32_t)offset, (int32_t)(inplace ? 1 : 0) };
...
static void ggml_compute_forward_get_rows_f16(
    ...
    for (int64_t i = ir0; i < ir1; ++i) {
        ...
        ggml_fp16_to_fp32_row(
                (ggml_fp16_t*)(const void *) ((char *) src0->data + i01*nb01 + i11*nb02 + i12*nb03),
                     (float *) ((char *)  dst->data + i10*nb1  + i11*nb2  + i12*nb3), nc);
    }
}
...
static void ggml_compute_forward_get_rows_bf16(
    ...
    for (int64_t i = ir0; i < ir1; ++i) {
       ...
       ggml_bf16_to_fp32_row(
                (ggml_bf16_t*)(const void *) ((char *) src0->data + i01*nb01 + i11*nb02 + i12*nb03),
                     (float *) ((char *)  dst->data + i10*nb1  + i11*nb2  + i12*nb3), nc);
    }
}
...
struct ggml_hash_set ggml_hash_set_new(size_t size) {
    ...
    result.size = size;
    result.keys = (ggml_tensor**)GGML_MALLOC(sizeof(struct ggml_tensor *) * size);
...
enum ggml_status ggml_graph_compute(struct ggml_cgraph * cgraph, struct ggml_cplan * cplan) {
    ...
    struct ggml_compute_state_shared state_shared = {
        ...
        /*.node_task               =*/ ATOMIC_INT_INIT(GGML_TASK_TYPE_FINALIZE),
        /*.abort_callback          =*/ NULL,
...
struct gguf_context * gguf_init_from_file(const char * fname, struct gguf_init_params params) {
    ...
    // read the kv pairs
    {
        ...
        ctx->header.n_kv = 0;
        ctx->kv = (gguf_kv*)GGML_CALLOC(n_kv, sizeof(struct gguf_kv));
    ...
    if (ctx->header.n_tensors > 0) {
        ctx->infos = (gguf_tensor_info*)GGML_CALLOC(ctx->header.n_tensors, sizeof(struct gguf_tensor_info));
